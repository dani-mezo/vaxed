...
class Task1:
    def __init__(self, excel):
        ...
        self.header_month_cell = None
        self.find_header_month(self.excel.hr_sheet)
        if not self.header_month_cell:
            logging.error("Nem találtam meg a referencia hónap oszlopát: '" + self.excel.header_month_column_name + "'")
            return
        logging.info('Referencia hónap oszlopát megtaláltam: ' + self.header_month_cell.column_letter)
        self.process_people()
    ...
    def color_promoted_cells(self):
        ...
        for person in self.excel.cells_promoted_for_coloring:
            ...
            WorksheetUtils.color_cell(self.excel.cells_promoted_for_coloring[person], color)
        logging.info('Színezés sikeresen befejzve!')

    def find_header_month(self, hr_sheet):
        logging.info("Referencia hónap oszlop értéke: '" + self.excel.header_month_column_name + "'")
        logging.info('Keresem a referencia hónap oszlopot...')
        for cell in hr_sheet[self.header_row_number]:
            if cell.value == self.excel.header_month_column_name:
                self.header_month_cell = cell
    ...
    def find_and_color_name_in_hr(self, hr_sheet, name):
        ...
        for row in hr_sheet.iter_rows():
            if row[self.name_column].value == name:
                ...
                reference_month_cell = row[self.header_month_cell.column - 1]
